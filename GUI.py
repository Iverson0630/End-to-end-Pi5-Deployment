...
class MainWindow(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Hip‑Exo Control v2.0 – RL")
    ...
    def _build_layout(self):
        ...
        # === 显示的 4 个旋钮 ===
        self.sb_k   = make_dspin(val=4.00, mn=0, mx=10, step=0.10, tip="Rescaling gain (0..10)")
        self.sb_kf  = make_dspin(val=2.00, mn=0, mx=10, step=0.10, tip="Flex_Assist_gain (0..10)")
        self.sb_ke  = make_dspin(val=2.00, mn=0, mx=10, step=0.10, tip="Ext_Assist_gain (0..10)")
        self.sb_p_on   = make_dspin(val=0.8, mn=-10, mx=10, step=0.05, tip="Gate power-on threshold p_on (on τ·ω)")
        self.sb_lead   = make_isin(val=35, mn=0, mx=200, step=5, tip="Gate lead (ms) — 预测超前时间")
        ...
        labels = ["Rescaling", "Flex_gain", "Ext_gain", "p_on", "lead (ms)"]
        spins  = [ self.sb_k,   self.sb_kf,    self.sb_ke,   self.sb_p_on, self.sb_lead ]
    ...
    def _tx_params(self):
        ...
        lead_ms     = int(self.sb_lead.value())
        ...
        # 头部：A5 5A len(17)   —— MCU 目前不使用 len，但保持一致
        header = struct.pack('<BBB', 0xA5, 0x5A, 20)   # ← 把 17 改回 20
